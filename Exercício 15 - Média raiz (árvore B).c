...
//Essa função não funicionava quando fiz o código, por isso usei outra
/*static NodeB *split_pag(NodeB *pai, int posF_cheio)
{
    int i;
...
int MediaRaiz(NodeB *tree)
{
    ...
    //É uma função simples para ver a media do primeiro vetor da árvore
    for(i = 0; i < tree->nro_chaves; i++)
    {
        if(tree->chaves[i] != NULL)
        {
            //Se o vertor for nulo em i, significa que houve uma divisão (split), então não deve contar para a média
            tot++;
            //Incremena o valor da soma para calcular posteriormante  a média

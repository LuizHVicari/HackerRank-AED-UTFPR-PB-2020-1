...
int ContarPaginas(NodeB *tree)
{
    if (tree != NULL)
    {
        //Função percorre a árvore recursivamente, o "for" entra em cada um de seus nós para passar para a função
        //A cada págia encontrada, ele aumenta o valor da variável soma 
        int soma = 1;
        for (int i = 0; i <= tree->nro_chaves; i++)
                soma = soma + ContarPaginas(tree->filhos[i]);

        return soma;
    }
    //Retorna 0 quando a àrvore estiver vazia
    return 0;
}
